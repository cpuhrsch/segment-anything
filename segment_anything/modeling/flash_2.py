...
def _fwd_kernel(
    ...
    b_offs_m = start_m + tl.arange(0, BLOCK_M)
    b_offs_n = tl.arange(0, BLOCK_N)
    ...
    for start_n in range(lo, hi, BLOCK_N):
        ...
        b = tl.load(B + b_offs_m[:, None]*stride_bm + (b_offs_n[None, :])*stride_bn,
                        mask=((b_offs_m[:, None] < N_CTX) & (b_offs_n[None, :] < N_CTX)),
                        other=0.0)
        qk += b
